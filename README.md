...
基于 `causis_api` 的 Wheel 策略回测项目。当前主实现为 **AZYC001001（卖 Put + ETF covered call，事件驱动版）**，策略逻辑与产品文档对齐 `PLAN.md`。

- 状态机：`FLAT -> SHORT_PUT -> LONG_ETF_SHORT_CALL -> FLAT`
- 信号按分钟触发，成交使用下一根 bar 的 `OPEN`
- 持仓期采用“事件驱动 + 日频估值”加速，不做全分钟扫描
- 输出订单、持仓、日频损益、指标与图表
...
- **到期处理**：
  ...
  - Call ITM：按执行价卖出 ETF
  - OTM：期权价值归零
- **流动性过滤**：信号 bar 与成交 bar 均要求 `VOLUME > 20`
...
| `strategy_id` | 策略编号 |
| `etf_symbol` | ETF 标的代码 |
| `option_code` | 期权链过滤代码（可选，默认由 `etf_symbol` 自动提取） |
| `start_date` / `end_date` | 回测区间 |
| `frequency` | 数据频率（如 `minute1`） |
| `fixed_contracts` | 每次卖出合约张数（仅 `contract_sizing=fixed` 时生效） |
...
4. 查看输出目录（默认在 `AZYC001001/outputs` 的相对路径）中的：
...
- 当前版本聚焦 **卖 Put + ETF 首版可运行框架**，不包含保证金/资金占用/强平等账户约束。
- 默认不做提前展期。
